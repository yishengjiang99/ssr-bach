...
var getNotesForOctave = function (octave) {
  return Object.keys(NOTES).reduce((obj, note) => {
    if (note.split('').pop() == +octave) obj[note] = NOTES[note];
    return obj;
  }, {});
};
  ...
  play() {
    ...
    this.interval = setInterval(() => {
      this.setState(
        {
          step:
            this.state.step < this.state.steps - 1 ? this.state.step + 1 : 0,
        },
        () => {
          var next = this.state.pads[this.state.step]
              ...
              <div className="pads">
                {group.map((pad, i) => (
                  <div
                    ...
                    className={`pad ${groupIndex === step ? 'active' : ''} ${
                      pad === 1 ? 'on' : ''
                    }`}
                  />
                ))}
