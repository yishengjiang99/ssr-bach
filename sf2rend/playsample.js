...
const parseSampleInfo = (attbag) => {
    const sr = parseInt(attbag.getAttribute("sr")), startloop = parseInt(attbag.getAttribute("startloop")), pitch = parseFloat(attbag.getAttribute("pitch")).toFixed(1), endloop = parseInt(attbag.getAttribute("endloop")), file = attbag.getAttribute("file"), range = attbag.getAttribute("range");
...
window.onmousedown = async (e) => {
    ...
    if (e.target.classList.contains("pcm")) {
        ...
        run_sf2_smpl(parseSampleInfo(attbag), zone, smplData, midi).then((triggerAdsrRelease) => {
