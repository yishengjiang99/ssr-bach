...
exports.SFZone = exports.SFGenerator = void 0;
...
class SFZone {
  constructor(ids) {
    ...
    this.reverbSend = 0;
    /* percent of signal to send back.. in 0.1% units*/

    this.pan = -1;
    /* shift to right percent */
    ...
    this._rootkey = -1;
    this.tuning = 0;
    this.sampleMode = _sfTypes.LOOPMODES.CONTINUOUS_LOOP;
    this.sampleID = -1;
    this.generators = [];
  ...
  get scaleTuning() {
    return this.generators[_sfTypes.sf_gen_id.scaleTuning] ? this.generators[_sfTypes.sf_gen_id.scaleTuning].s16 : 0;
  }

  get keynumToVolEnvDecay() {
    return this.generators[_sfTypes.sf_gen_id.keynumToVolEnvDecay] ? this.generators[_sfTypes.sf_gen_id.keynumToVolEnvDecay].s16 : 0;
  }
  ...
  get sample() {
    return _objectSpread(_objectSpread({}, this._shdr), {}, {
  ...
  mergeWith(zoneb) {
    ...
    for (var g of Object.values(zoneb.generators)) {
  ...
  increOrSet(gen) {
    if (!this.generators[gen.operator]) this.generators[gen.operator] = gen;else this.generators[gen.operator].s16 += gen.s16;
  }
  ...
  applyGenVal(gen, from) {
    switch (gen.operator) {
      ...
      case attackVolEnv
      /*This is the time, in absolute timecents, from the end of the Volume
      ...
      case endloopAddrsCoarse:
        this.sampleOffsets[3] += gen.s16 << 15; // this._shdr.endLoop += 15 << gen.s16;
      ...
      case sampleID:
        ...
        if (this.sampleID != -1) {//throw 'applying to existing sample id';
        }
