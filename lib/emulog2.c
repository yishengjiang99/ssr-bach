 ...
 *
 */
int snd_sf_linear_to_log(unsigned int amount, int ratio, int offset) {
  int v;
  int s, low, bit;

  if (amount < 2) return 0;
  for (bit = 0; !(amount & 0x80000000L); bit++) amount <<= 1;
  s = (amount >> 24) & 0x7f;
  low = (amount >> 16) & 0xff;
...
int main(int argc, char **argv) {
  for (int i = 80000; i > 1110; i *= .8)
    printf("calc %d %x\n", i, calc_rate_offset((int)i));
  return 1;
}