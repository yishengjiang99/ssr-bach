...
class MyAudioProcessor extends AudioWorkletProcessor {
    ...
    msghandler({ data: { iteratorParams, statebuff, sdtaF32s } }) {
    ...
    process(_, outputList, parameters) {
        //@ts-ignore
        const currentFrame = globalThis.currentFrame;
        if (!this.sdtaF32s || !this.statebuff)
            return true;
        this.iterators.forEach((iterator) => {
            if (iterator.start > currentFrame)
                return;
            const pitchShift = parameters["pitchShift"];
            const egVal = parameters["egVal"];
            function modAmp(val, i) {
            ...
            for (let i = 0; i < outputList[0][0].length; i++) {
                // @ts-ignore
                const v1 = lerp(this.sdtaF32s[iterator.pos], this.sdtaF32s[iterator.pos + 1], iterator.frac);
