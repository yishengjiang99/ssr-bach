import {
  ...
  IBag,
  generatorNames,
} from '../node_modules/parse-sf2/dist/index.js'; //
import { readAB } from '../node_modules/parse-sf2/dist/aba.js'; //

const rendfile = async (file: string) => {
  const { pdtaBuffer, sdtaStream, nsamples, infos } = await sfbkstream(file);

  const pdta = new PDTA(readAB(pdtaBuffer));
  let sampleIdx;
  const ibag2pbag = {};
  function printPbags(ib: IBag) {
    ...
    return h(
      'div',
      {},
      ibag2pbag[ib.izone.ibagId].map((pb) =>
        h(
          'span',
          {},
  ...
  const presets = pdta.pbag.reduce((inst2pb, pb, pbidx) => {
    if (pb?.pzone?.instrumentID > -1) {
      inst2pb[pb.pzone.instrumentID] = inst2pb[pb.pzone.instrumentID] || [];
