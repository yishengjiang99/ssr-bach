...
let seeklock = null, sliderlocks = {};
let worker, ctx;
const aggregatedFeed = new TransformStream();
export const UISlider = ({ parent = "#stats", worker, cmd = "config", attribute, label = "", defaultValue = 12, min = -12, max = 12, step = 0.1, }) => {
    worker = worker;
...
export const postSeek = async (val) => {
    ...
    await new Promise((r) => {
        wschan.addEventListener("message", ({ data: { ack } }) => {
            if (ack === "seek") {
                r(1);
            }
        }, { once: true });
