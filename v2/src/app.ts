...
export default function httpd(port) {
        ...
        .map((z) => {
          ...
          res.write(
            `<tr><td>${[
              ...
              z.sample?.sampleRate,
              z.sample?.name,
              `<a class="pcm" href="/sample/${m[2]}/${m[1]}/${z.keyRange.hi - 5}/${
                z.velRange.lo + 5
              }">play</a>`,
            ].join("</td><td>")}</td> </tr>`
          );
        ...
        <body class="container row">
          <aside class="col-md-3">
            ...
            <ul class="list-group" style="max-height: 15vh; overflow-y: scroll">
              ${tones.map((p) => presetlink(p))} ${drums.map((p) => presetlink(p))}
            </ul>
          </aside>
...
function midilink(file: string): string {
  ...
  <a class='pcm' onclick='start("/pcm/${encodeURIComponent(file)}")'  href='#'>${file}</a>
  </li>`;
}
