...
export class SF2File {
  ...
  _render(channel: sfTypes.Channel, outputArr: Buffer, blockLength) {
    ...
    for (let offset = 0; offset < blockLength; offset++) {
      ...
      const amp = newVal * channel.envelope.shift() * channel.gain;

      const op = channel.pan;
      while ((op >> 1) * (op >> 1) < op);
      let L = currentVal + amp - channel.pan;
      let R = currentVal + amp + channel.pan;

      L = clamp(compression(L, 0.3, 3, 0.9), -1, 1);
      R = clamp(compression(R, 0.3, 3, 0.9), -1, 1);
      outputArr.writeFloatLE(L, outputByteOffset);
      outputArr.writeFloatLE(R, outputByteOffset);
