...
for (const name of execSync("cat " + csvfile + " |cut -f1 -d','|sort |uniq")
    ...
    execSync(`curl "${sfUrl(setname, fontname)}" -o - |grep 'data:audio/mp3;base64,' |awk -F 'data:audio/mp3;base64,' '{print $2}'|tr '\",' '\n'|grep -v ^$ |base64 --decode > ${localname}`);
    const byteswrote = parseInt(execSync("wc -c " + localname)
        .toString()
        .trim()
        .split(/\s+/)[0]);
    const bytesPerNote = byteswrote / 88;
    const uniqnotes = execSync(`grep ${name} ${csvfile} |cut -f2 -d','|sort |uniq|tr '\n' ','`)
        .toString()
        .trim()
        .split(",");
