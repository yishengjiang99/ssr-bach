...
export async function initsfbk(url: string, port?: MessagePort): Promise<SFBKRet> {
	const ab = await (
		...
		if (res.body.getReader) {
			...
			reader.read().then(function process({ done, value }) {
				...
				for (let i = 0; i < value.length; i += 2) {
					...
					floats[offset++] = int >= 0x8000 ? -(0x10000 - int) / 0x8000 : int / 0x7fff;
				}
...
export function getSampleIzones(sid: number, pdta: PDTA): SFZone[] {
	return pdta.ibag.filter((ib) => ib.izone.sampleID == sid).map((ib) => ib.izone);
}
