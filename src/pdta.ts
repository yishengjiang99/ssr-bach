...
type findPresetFnType = (pid: number, bank_id?: number, key?: number, vel?: number) => SFZone[];
...
export class PDTA {
	...
	findPreset: findPresetFnType = (pid: number, bank_id = 0, key = -1, vel = -1) => {
		...
		return pbag
			...
			.map((pbg) => {
				...
				return izones.map((iz) =>
					makeRuntime(iz, defaultBg, pbg, presetDefault.pzone, shdr[iz.sampleID])
				);
			})
			.flat();
	};
		...
		do {
			...
			switch (sectionName) {
				...
				case "ibag": {
					let ibginst = 0;
					for (let i = 0; i < sectionSize; i += pbagLength) {
						if (this.iheaders[ibginst + 1] && i >= this.iheaders[ibginst + 1].iBagIndex) ibginst++;
						this.ibag.push({
							igen_id: r.get16(),
							imod_id: r.get16(),
	...
	private psh(ibginst: number, i: number, pbagLength: number) {
		this.iheaders[ibginst].ibags && this.iheaders[ibginst].ibags?.push(i / pbagLength);
	}
