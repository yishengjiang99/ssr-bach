...
// based on midi-file, but less judgemental.
//
function getMeta(buffer, type = 0x00) {
  const p = new Parser(buffer);
  const m = [];
  while (!p.oef) {
    while (p.readUInt8() != 0xff && p.readUInt8() !== type);
    const l = p.readVarInt();
    m.push(p.readString(l));
  }
...
function parseMidi(data) {
  const [header, ...tracks] = data
    ...
    .map((chunk, i) => {
      if (i === 0) {
        return parseHeader(Buffer.from(chunk.split("MThd")[1] || ""));
      } else {
        var p = new Parser(Buffer.from(chunk));
        var length = p.readUInt32();
        var data = p.readBytes(length);
...
export { parseHeader, parseMidi, getMeta, Parser };
