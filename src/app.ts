...
import { uint8sf2 } from './pdta';
import { sleep } from './utilv1';
import { readFileSync } from 'fs';

uint8sf2(new Uint8Array(readFileSync('./file.sf2'))).then(
  ({ pdta, sdta, render, runtime }) => {
    //console.log('sfread');
          ...
          pdta.pgen.map((p, i) => {
            ...
            if (
              ...
              if (
                ...
                phdrId++;
                hdr +=
                  '<br>--' + pdta.phdr[phdrId].name + ' ' + phdrId + '<br>';
                for (let i = 40; i < 70; i++) {
                  hdr += `&nbsp;<a class='pcm' href="runzone/${pdta.phdr[phdrId].presetId}}/${i}/55/${pdta.phdr[phdrId].bankId}">${keys88[i]}</a>`;
                  if (i % 12 == 0) {
                    hdr += '<br>';
                  }
                }
