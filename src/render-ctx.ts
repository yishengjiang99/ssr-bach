...
export class RenderCtx {
	...
	pushToProduction() {
		while (this.productionNow.length) {
			...
			if (keyoff == 1 && this.voices[channelId]) {
				...
				this.voices[channelId].length = this.voices[channelId]?.mods.ampVol.stages[4];
	...
	constructor(private sff: SF2File) {
		...
		this.inputs = Buffer.from(this.sff.sdta.data);
	}
		...
		for (let offset = 0; offset < blockLength; offset++) {
			...
			if (offset > 1) {
				const [vm1, v0, v1, v2] = [-1, 0, 1, 2].map((i) =>
					this.inputs.readFloatLE((iterator + i) * 4)
				);
				newVal = hermite4(shift, vm1, v0, v1, v2);
			} else {
				newVal = this.inputs.readFloatLE(iterator * 4);
			}

			let sum = currentVal + newVal * volume;

			outputArr.writeFloatLE(sum * voice.staticLevels.pan.left, outputByteOffset);
			outputArr.writeFloatLE(sum * voice.staticLevels.pan.right, outputByteOffset + 4);
	...
	render(blockSize) {
		...
		loop(blockSize * 2, (n) => ob.writeFloatLE(0, n * Float32Array.BYTES_PER_ELEMENT));
