...
onmessage = (e) => {
  ...
  if (port) {
    port.postMessage({ sab: _heap.subarray(sd.readOffset, blocksize) });
  }
...
export class RenderCtx {
  ...
  fps: number;
  voices: any[];
  staging: { queue: any[]; index: number };
  _masterVol: number;
  _chanVols: any[];
  ...
  _render(voice, outputArr, blockLength, n) {
